...
public class Calculator {
    ...
    public static String parse(String expression) throws Exception {
        ...
        if (Roman.isRoman(operands[0]) && Roman.isRoman(operands[1])) { //Проверка римских чисел
            number1 = Roman.convertToArabian(operands[0]);
            number2 = Roman.convertToArabian(operands[1]);
            isRoman = true;
        }
        else if (!Roman.isRoman(operands[0]) && !Roman.isRoman(operands[1])) { //Проверка арабских чисел
